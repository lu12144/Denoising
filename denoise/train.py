...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate((train_loader), 0):
        ...
        to_check += 1
        if to_check % 1000 == 0:
            print('have finished %d' % to_check)
        ...
        # print(restored)
        # Compute loss at each stage  .cpu().detach().numpy()
        # loss = torch.from_numpy(np.sum([criterion(torch.clamp(restored[j],0,1),target) for j in range(len(restored))]))

        loss = torch.tensor([0],dtype=torch.float64).cuda()
        for j in range(len(restored)):
            loss += criterion(torch.clamp(restored[j],0,1),target)

        # loss  = torch.tensor(loss) # xiu 
